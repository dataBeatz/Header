  ...
  for (let i = 1; i < 51; i++) {
    const generateArtistData = function () {
      for (let j = start; j < stop + 1; j++) {
        artist +=
        `${faker.name.findName ()},${Math.round (Math.random ()) ? true : false},${Math.round(Math.random() * 0.7 + 0.3) ? true : false},${faker.lorem.paragraphs(1)},"[${createImagesArray()}]"\n`
      }
    };
    generateArtistData ();
    let writeStream = fs.createWriteStream (`./artistTableData.csv`, {"flags": "a"});
